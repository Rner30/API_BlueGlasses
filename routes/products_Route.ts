...
router.get(
	"/:id",
	[check("id", "No es un ID valido de Mongo").isMongoId(), validateFields],
	getOneProduct
);
...
router.get(
	"/category/:name",
	[
		check("name", "Se necesita un nombre para realizar la busqueda")
			.not()
			.isEmpty(),
	],
	getAllItemsOfOneCategory
...
router.post(
	...
	[
		...
		check("stock", "El stock debe ser un valor numerico")
			.isNumeric()
			.not()
			.isString(),
